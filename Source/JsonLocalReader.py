...
for key, val in monument_list[156].items():
	if key == "title":
		monument_list[156]['title'] = WikidataItem(get_entity_dict_from_api(str(val))).get_label()
	elif key == "id":
		monument_list[156]['id'] = WikidataItem(get_entity_dict_from_api(str(val))).get_label()


for key in monument_list[156]['claims'].keys():
	property_list.append(WikidataProperty(get_entity_dict_from_api(key)).get_label())
final_monument_list = dict(zip(property_list, list(monument_list[156]['claims'].values())))
monument_list[156]['claims'].clear()
monument_list[156]['claims'].update(final_monument_list)
property_list = []

	
for key, val in monument_list[156]['claims'].items():
	for values in val:
			...
			if key1 == "references":
				for i in range(len(values['references'])):
					...
					for key2, val2 in values['references'][i]['snaks'].items():
						#print(values['references'][i]['snaks'][key2])
